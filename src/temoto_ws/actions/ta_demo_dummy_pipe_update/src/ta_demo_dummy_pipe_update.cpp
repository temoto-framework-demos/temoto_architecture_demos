...
#include "ta_demo_dummy_pipe_update/temoto_action.h"
#include "ta_demo_dummy_pipe_update/macros.h"

#include "temoto_component_manager/component_manager_interface.h"

/* 
 * ACTION IMPLEMENTATION of TaDemoDummyPipeUpdate 
 */
class TaDemoDummyPipeUpdate : public TemotoAction
{
public:

// Constructor. REQUIRED BY TEMOTO
TaDemoDummyPipeUpdate()
{
...
void executeTemotoAction()
{
  ...
  cmi_.registerPipeStatusCallback(&TaDemoDummyPipeUpdate::pipeStatusCb);
  cmi_.registerPipeUpdateCallback(&TaDemoDummyPipeUpdate::pipeUpdateCb);

  // Start the pipe and get the output topics of the last pipe segment
  TEMOTO_INFO("Starting the dummy_pipe pipe ...");
  temoto_core::TopicContainer pipe_topics = cmi_.startPipe("dummy_pipe");

  // Get the name of the topic where camera feed is published
  std::string ppc_data_topic = pipe_topics.getOutputTopic("data_type_4");
...
temoto_component_manager::ComponentManagerInterface<TaDemoDummyPipeUpdate> cmi_;

}; // TaDemoDummyPipeUpdate class

/* REQUIRED BY CLASS LOADER */
CLASS_LOADER_REGISTER_CLASS(TaDemoDummyPipeUpdate, ActionBase);
